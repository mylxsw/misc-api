...
def rewrite_image_srcs(html: str, resolve) -> tuple[str, int]:
    """Rewrite every <img> src in ``html`` using the ``resolve`` callback.

    Operating on the parsed DOM (rather than string replacement) avoids three
    classes of bug that plague ``html.replace(src, url)``:
      - HTML-escaped attribute values: a src with ``&`` appears as ``&amp;`` in
        the serialized html, so a decoded src never matches literally.
      - substring collisions: one image URL being a prefix of another causes
        the shorter replacement to corrupt the longer.
      - duplicate work: the same src is resolved once and applied to every
        matching <img>.

    Args:
        html: WeChat body HTML.
        resolve: callable(src) -> hosted_url_or_None. Return None to leave the
            image untouched (e.g. it is not an uploadable image). Exceptions
            propagate to the caller.

    Returns (new_html, rewritten_count) where rewritten_count is the number of
    distinct srcs that were rewritten.
    """
    soup = BeautifulSoup(html, "html.parser")
